...
import br.com.mateusulrich.recipeservice.api.dtos.recipe.retrieve.data.IngredientData;
...
public class ListRecipeResponse {
    ...
    public static ListRecipeResponse fromEntityToListResponse(Set<Integer> ingredients, Recipe recipe) {
        int count = 0;
        Set<IngredientData> ingredientDataSet = (recipe.getRecipeIngredients() != null && !recipe.getRecipeIngredients().isEmpty())
                ? recipe.getRecipeIngredients().stream().map(IngredientData::fromIngredient).collect(Collectors.toSet())
                : Set.of();
